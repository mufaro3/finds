...
def calculate_update_rk4(system: NDArray, time_step: float) -> NDArray:
    ...
    .. math::
        \dot{\mathbf{X}} \approx \frac{\dot{\mathbf{X}}_1 + 2 \dot{\mathbf{X}}_2 + 2 \dot{\mathbf{X}}_3 + \dot{\mathbf{X}}_4}{6}
    ...
    Before returning, the orientation vectors need to be renormalized, and this is done through :py:func:`src.calculations.normalize_orientation_vectors`.
    """
    X0 = system
    f  = calculate_system_derivative
...
def perform_simulation(
    ...
    :param print_to_file: Whether or not to save the path generated from the simulation to an autogenerated CSV file, located at `output/test-(timestamp)/path.csv`.
    :type  print_to_file: bool

    :param generate_animation: Whether or not to use the autogenerated CSV file to generate an animation as well. Requires :code:`print_to_file` to be True to work.
    ...
    output_filename = output_dir / "path.csv" 
    ...
    for simulation_index in range(total_iterations):
        ...
        system = calculate_update_rk4(system, time_step)
