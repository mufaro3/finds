...
def test_calculate_feature_velocities():
    ...
    for _ in range(NUMBER_OF_TESTS):
        ...
        assert np.all(np.isfinite(feature_velocities)), \
            'Feature velocities matrix contains an infinite value.'

        assert np.all(np.isreal(feature_velocities)), \
            'Feature velocities matrix contains a complex value.'
...
def test_calculate_system_derivative():
    ...
    for _ in range(NUMBER_OF_TESTS):
        ...
        assert np.all(np.isfinite(derivative)), \
            'Derivative matrix contains an infinite value.'

        assert np.all(np.isreal(derivative)), \
            'Derivative matrix contains a complex value.'
